...
from .configuration_langtpp import LanguageTPPConfig
...
_CONFIG_FOR_DOC = "LanguageTPPConfig"
...
class Qwen2RotaryEmbedding(nn.Module):
    def __init__(
        ...
        scaling_factor=1.0,
        rope_type="default",
        config: Optional[LanguageTPPConfig] = None,
    ):
        super().__init__()
...
class Qwen2Attention(nn.Module):
    ...
    def __init__(self, config: LanguageTPPConfig, layer_idx: Optional[int] = None):
        super().__init__()
        self.config = config
...
class Qwen2DecoderLayer(nn.Module):
    def __init__(self, config: LanguageTPPConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
...
class Qwen2PreTrainedModel(PreTrainedModel):
    config_class = LanguageTPPConfig
...
class Qwen2Model(Qwen2PreTrainedModel):
    ...
    def __init__(self, config: LanguageTPPConfig):
...
class LanguageTPPForCausalLM(Qwen2PreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        # # TPP relevant definitions transformer hawkes
        self.temporal_head = nn.Linear(config.hidden_size, config.hidden_size, bias=False)
        self.next_event_time_interval_head = nn.Linear(config.hidden_size, 1, bias=False)

        max_len = 5000
        position = torch.arange(0, max_len).float().unsqueeze(1)
        # [model_dim //2 ]
        div_term = (torch.arange(0, config.hidden_size, 2).float() * -(math.log(10000.0) / config.hidden_size)).exp()

        self.temporal_layer_time_delta = nn.Linear(1, config.hidden_size // 2, bias=False)
        self.register_buffer("temporal_position", position)
        self.register_buffer("temporal_div_term", div_term)
        self.num_types = 1  # leave type prediction to LLM
        self.temporal_eps = torch.finfo(torch.float32).eps
        self.temporal_mu = nn.Sequential(
            nn.Linear(config.hidden_size, self.num_types),
        )
        self.temporal_eta = nn.Sequential(
            nn.Linear(config.hidden_size, self.num_types),
        )
        self.temporal_gamma = nn.Sequential(
            nn.Linear(config.hidden_size, self.num_types),
        )
    ...
    def forward(
        ...
        event_times: Optional[torch.FloatTensor] = None,
        time_token_id=None,
        *args,
        **kwargs,
        ...
        ```python
        >>> from transformers import AutoTokenizer, LanguageTPPForCausalLM

        >>> model = LanguageTPPForCausalLM.from_pretrained(PATH_TO_CONVERTED_WEIGHTS)
        ...
        if event_times is not None:
            non_pad_mask = self.get_non_pad_mask(event_times)
            # dummy event type
            event_type = torch.ones_like(event_times) * non_pad_mask.squeeze(-1)
            time_gap = torch.cat(
                (
                    torch.zeros_like(event_times[:, 0:1]),
                    (event_times[:, 1:] - event_times[:, :-1]) * non_pad_mask[:, 1:].squeeze(-1),
                ),
                axis=-1,
            )  # batch_size, seq_len
        ...
        if (labels is not None and event_times is None) or not self.training:
            ...
            if labels is not None and event_times is None:
                ...
                loss = llm_loss
        if event_times is not None:
            assert llm_loss == 0 or not self.training
            with torch.no_grad():
                # decoder outputs consists of (dec_features, layer_state, dec_hidden, dec_attn)
                outputs = self.model(
                    input_ids=None,
                ...
                # logits = self.lm_head(hidden_states[:, -num_logits_to_keep:, :])
                logits = None
                # DO NOT BP on temporal_hidden_states
                assert time_token_id is not None
                temporal_hidden_states_list = [
                    hidden_states[i, :-1, :][index[1:], :] for i, index in enumerate(input_ids == time_token_id)
                ]  # index - 1 due to label shift
                temporal_hidden_states = pad_sequence(temporal_hidden_states_list, batch_first=True, padding_value=0)

            temporal_hidden_states = self.temporal_head(temporal_hidden_states)  # projection

            mle_loss = self.compute_loss_mle(
                temporal_hidden_states,
                event_type,
                event_times,
                time_gap,
                non_pad_mask,
                num_grid=self.config.temporal_mle_num_grid,
            )
    ...
    ###### adding the following code for procssing TPP
    def temporal_enc(self, time):
        """
        Input: batch*seq_len.
        Output: batch*seq_len*d_model.
        """
        non_pad_mask = (~(time == -1)).unsqueeze(2)
        if not hasattr(self, "position_vec"):
            self.position_vec = torch.tensor(
                [math.pow(10000.0, 2.0 * (i // 2) / self.config.hidden_size) for i in range(self.config.hidden_size)],
    ...
    def get_intensity(self, t, hidden_state, event_time, time_gap, non_pad_mask):
        # t size: batch*len-1*num_samples
        if t.ndim == 2:
            t = t.unsqueeze(2)
        assert t.ndim == 3

        self.affect = self.affect_layer(hidden_state)
        self.base = self.base_layer(hidden_state)

        intensity = self.intensity_layer(
            self.affect[:, :-1, None, :] * t.unsqueeze(3) + self.base[:, :-1, None, :]
        ).squeeze(3)  # (batch*len-1*1/num_samples)
        all_lambda = intensity * non_pad_mask[:, :-1, None, :]

        return all_lambda

    def time_shifted_pe(self, x, interval):
        """
        ...
        """
        phi = self.temporal_layer_time_delta(interval.unsqueeze(-1))
        ...
        arc = (self.temporal_position[:length] * self.temporal_div_term).unsqueeze(0)
    ...
    def get_non_pad_mask(self, seq):
        ...
        TIME_PAD_ID = -1
        assert seq.dim() == 2
        return seq.ne(TIME_PAD_ID).type(torch.float).unsqueeze(-1)

    def compute_loss_mle(self, enc_output, event_type, event_time, time_gap, non_pad_mask, num_grid=10):
        cell_t = self.state_decay(encode_state=enc_output[:, :], duration_t=time_gap[:, :, None]).squeeze(-2)
        ...
        assert (
            lambda_at_event.shape[1]
            == lambda_t_sample.shape[1]
            == time_gap.shape[1] - 1
            == non_pad_mask.shape[1] - 1
            == type_mask.shape[1] - 1
        )
    ...
    def make_dtime_loss_samples(self, time_delta_seq, num_grid):
        ...
        """
        # [1, 1, n_samples]
        dtimes_ratio_sampled = torch.linspace(start=0.0, end=1.0, steps=num_grid)[None, None, :].to(
            time_delta_seq.device
        )
    ...
    def compute_states_at_sample_times(self, encode_state, sample_dtimes):
        ...
        cell_states = self.temporal_mu(encode_state[:, :-1, None, :]) + (
            # self.eta(encode_state[:,:-1, None, :]) - self.temporal_mu(encode_state[:,:-1, None, :])) * torch.exp(
            self.temporal_eta(encode_state[:, :-1, None, :]) - self.temporal_mu(encode_state[:, :-1, None, :])
        ) * F.softplus(-self.temporal_gamma(encode_state[:, :-1, None, :]) * sample_dtimes[:, 1:, :, None])
    ...
    def compute_loglikelihood(self, time_delta_seq, lambda_at_event, lambdas_loss_samples, seq_mask, lambda_type_mask):
        ...
        # [batch_size, seq_len]
        event_lambdas = torch.sum(lambda_at_event * lambda_type_mask, dim=-1) + self.temporal_eps
        # mask the pad event
    ...
    def state_decay(self, encode_state, duration_t):
        ...
        states = self.temporal_mu(encode_state[:, :-1]) + (
            # self.eta(encode_state[:,:-1]) - self.temporal_mu(encode_state[:,:-1])) * torch.exp(
            self.temporal_eta(encode_state[:, :-1]) - self.temporal_mu(encode_state[:, :-1])
        ) * F.softplus(-self.temporal_gamma(encode_state[:, :-1]) * duration_t[:, 1:])
